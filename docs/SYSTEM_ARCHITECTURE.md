...
- `frontend/`: React + TypeScript + Vite application shell with multi-view navigation, SaaS-style landing structure, interactive hero cover integration, CSV/PDF upload support, analytics summary UI, tables, custom SVG charts with empty states, saved analyses panel, saved analysis detail dashboard, responsive layout polish, reduced-motion-aware interactions, and HTML report action.
...
The frontend owns the current CSV/PDF selection, validation/extraction status, analytics summary workflow, saved analyses history foundation, saved detail dashboard, HTML report action, multi-view navigation, and responsive presentation layer. Its SaaS-style layout now applies the inspected Stitch analytics platform package as a visual reference while keeping original CampusInsight copy, existing data flow, and project-owned assets. The implementation adapts Stitch concepts such as pale analytics surfaces, section cards, compact navigation, chart panels, saved detail surfaces, and report action blocks without copying third-party branding, static mock data, unsupported actions, assets, colors, or text. It displays a dedicated home experience with the project hero cover image, validation status, GPA and credit summary cards, semester and course tables, grade distribution, course score visualizations, safe course risk review, saved analysis metadata summaries, full saved dashboards from stored JSON, and a report link for loaded saved details. Charts use deterministic backend analytics only and are rendered as responsive SVG components so CSV, PDF, and saved PDF detail dashboards do not depend on runtime container measurement. Empty states appear only when chart data is unavailable, and tables remain available as an accessible fallback.
